from RoveComm_Python.rovecomm import RoveComm, RoveCommPacket
import math
import time
import socket
import numpy as np
...
class Rover:
    def __init__(self, robot, rovecomm_node):
        ...
        self.leftFrontMotor = self.robot.getDevice("motor1")
        self.rightFrontMotor = self.robot.getDevice("motor2")
        self.leftBackMotor = self.robot.getDevice("motor3")
        self.rightBackMotor = self.robot.getDevice("motor4")
    ...
    def drive_callback(self, packet):
        ...
        self.rightFrontMotor.setVelocity((right) * self.MAX_SPEED)
    ...
    def drive_watchdog_check(self):
        ...
        if current_milli_time() - self.watchdog_timer > self.UPDATE_RATE * 1.5:
            ...
            self.rightFrontMotor.setVelocity(0)
